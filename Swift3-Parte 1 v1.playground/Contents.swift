...
let stringFormatada = String(format: "%.2f", 10.12334231)

// Inteiros
let idade = 24 //como eu não disse qual o tipo ele infere que é do tipo Int
let maiorIdade: Int = 18 //Aqui explicito que é Int
...
// Bool
var isOpen = false
var isClosed = !isOpen //Tô dizendo que é não falso

// Interpolando valores
let nome = "Tiago"
var stringComValores = "Meu nome é \(nome) e minha idade é \(idade)" //assim que se mistura variáveis com strings \(NomeDavariável)

// Convertendo valores
let inteiroParaString: String = String(10) //passei de Int para String
let doubleParaString: String = String(5.76) //passei de Float para String
...
// Conversão direta
let doubleFromInt: Double = Double(inteiro) //de Int quero passar para Double

let intFromDouble: Int = Int(double) //de Double quero passar para Int

let stringFromInt: String = String(inteiro) //quero passar de String para Int
let stringFromDouble: String = String(double)


// Conversão condicional
let intFromString: Int = Int(string)! //Ele vai forcar mas senão tiver um número vai haver um erro
...
var resposta: Int //resposta vai aceitar valores inteiros

// 1. Quanto é 2 elevado a 8?
resposta = Int (pow(2.0, 8.0))// utilize a função pow (base em Double, expoente em Double)
...
resposta = Int (sqrt(196.0))// utilize a função sqrt
...
let frase = "Na escala de tempo geológico, o \(periodoGeológico.0) " +
...
let index = nomes.index(of: "Lucas")

// 3. Remova o nome Lucas
nomes.remove(at: index!) //a funcao index(of:variavel) retorna um opcional, se Lucas nao tiver no array essa funcao recebe valor null
// ela só remove Lucas do array se o valor de index não for nulo
...
class MotorDeCarro: Gasolina {
    ...
    var quantidadeDeCombustível: Int = 60
...
class MotorDeCaminhão: Diesel {
    ...
    var quantidadeDeCombustível: Int = 90
    
    func injetarArAquecido() {<#code#>}
    
    func injetarCombustívelSobPressão() {<#code#>}
}
